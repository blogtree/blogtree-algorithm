 ...
 */
@Slf4j
public class BinaryTreeTraversalByRecursion {
     ...
     */
    @Test
    public void dlrByRecursionTest() {
        List<Integer> nodeList = new LinkedList();
        dlrByRecursion(getBinaryTreeRoot(), nodeList);

        log.info("dlrByRecursion, nodeList={}", nodeList);
        List<Integer> checkList = Arrays.asList(1, 2, 4, 5, 3, 6, 7);
        assert checkList.equals(nodeList);
    }

    private void dlrByRecursion(TreeNode node, List<Integer> nodeList) {
        if (node != null) {
            nodeList.add(node.val);
            dlrByRecursion(node.left, nodeList);
            dlrByRecursion(node.right, nodeList);
        }
    }
